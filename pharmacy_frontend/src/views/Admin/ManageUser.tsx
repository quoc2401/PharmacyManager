...
const ManageUser: FC = () => {
  ...
  const customFilter = (field: UserField,placeholder: string) => {
    return <InputText value={filters[field].value} 
  ...
  const leftToolbarTemplate = () => {
    return (
        <>
            <Button label="New" icon="pi pi-plus" className="rounded-md mr-2" onClick={openNew} />
            <Button label="Delete" icon="pi pi-trash" className="p-button-danger rounded-md mr-2" onClick={confirmDeleteSelected} 
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex">
          <span className="p-input-icon-left">
            ...
            <InputText placeholder="Keyword Search" onChange={onGlobalFilterChange}/>
          </span>
  ...
  return (
    <div className="grid-cols-12">
      <div className="card">
          ...
          >
            ...
            <Column
              ...
              filterElement={customFilter(UserField.USERNAME, "Search by Username...")}
            />
            <Column
              field="first_name"
              header="First Name"
              ...
              filter
              showFilterMenu={false}
              filterElement={customFilter(UserField.FIRST_NAME, "Search by First Name...")}
            />
            <Column
              field="last_name"
              header="Last Name"
              ...
              filter
              showFilterMenu={false}
              filterElement={customFilter(UserField.LAST_NAME, "Search by Last Name...")}
            />
            <Column
              field="birth_date"
              header="Birth Date"
            ...
            <Column
              ...
              showFilterMenu={false}
              filterType="number"
              filterElement={customFilter(UserField.PHONE, "Search by Phone Number...")}
            />
            <Column
              field="user_role"
              header="Role"
      ...
      <Dialog visible={newDialog} header="New User" modal 
              className='p-fluid w-[40%]' footer={newDialogFooter(saveUser, hideNewDialog)} 
              onHide={hideNewDialog}>
        <div className='field mb-5'>
          <label htmlFor="username">Username</label>
          <InputText  id="username" name='username' value={user.username} required 
                      autoFocus placeholder="Enter username..." className='rounded-md'
                      onChange={e => handleInput(e.target.value, UserField.USERNAME)}/>
        </div>
        <div className='field mb-5'>
          <label htmlFor="password">Password</label>
          <InputText  id="password" name='password' value={user.password} required 
                      placeholder="Enter password..." className='rounded-md'
                      onChange={e => handleInput(e.target.value, UserField.PASSWORD)}/>
        </div>
        <div className='flex mb-5 space-x-2'>
          <div className='field'>
            <label htmlFor="first_name">First Name</label>
            <InputText  id="first_name" name='first_name' value={user.first_name} required 
                        placeholder="Enter First Name..." className='rounded-md'
                        onChange={e => handleInput(e.target.value, UserField.FIRST_NAME)}/>
          </div>
          <div className='field'>
            <label htmlFor="last_name">Last Name</label>
            <InputText  id="last_name" name='last_name' value={user.last_name} required 
                        placeholder="Enter Last Name..." className='rounded-md'
                        onChange={e => handleInput(e.target.value, UserField.LAST_NAME)}/>
          </div>
        </div>
        <div className='field mb-5'>
          <label htmlFor="birth_date">Birth Date</label>
          <Calendar id="birth_date" name="birth_date" 
                    value={new Date(Date.parse(user.birth_date || ""))}
                    showIcon dateFormat='yy-mm-dd' placeholder="Choose Birth Date..."
                    onChange={e => handleInput(formatDate(e.target.value), UserField.BIRTH_DATE)}/>
        </div>
        <div className='field mb-5'>
            <label htmlFor="phone">Phone</label>
            <InputText  id="phone" name='phone' value={user.phone || ""} required 
                        placeholder="Enter Phone number..." className='rounded-md'
                        onChange={e => handleInput(e.target.value, UserField.PHONE)}/>
        </div>
        <div className='field mb-5'>
            <label htmlFor="user_role">User Role</label>
            <Dropdown id="user_role" name="user_role" value={user.user_role} 
                      options={roles} optionLabel="name"
                      optionValue="code" onChange={e => handleInput(e.target.value, UserField.USER_ROLE)}/>
        </div>
      </Dialog>
